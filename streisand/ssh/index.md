...
1. Enter `52.15.254.140` in the Host Name field.
1. Enter `443` in the Port field.
   * Port 22 is available as an option if your network does not block it.
...
1. Enter `8080` in the *Source port* field.
1. Make sure the `Auto` and `Dynamic` radio buttons are selected.
1. Click *Add* to add the tunnel. You should see `D8080` in the Forwarded ports box.
...
1. Enter `streisand-demo-site` in the first *Saved Sessions* box and click the *Save* button. The next time you launch PuTTY you can choose the session and click *Load* to restore all of these settings.
...
1. Enter `127.0.0.1` and Port `8080` on the *SOCKS Host* line.
...
1. Add a new entry to your `.ssh/config` file. It should look like this. Port 22 is available if your network does not block it. Be sure to adjust the location of the IdentityFile:
...
1. Tap *Settings*.
1. Tap *Host name* and enter `52.15.254.140`.
1. Tap *User Name* and enter `forward`.
1. Tap *Port* and enter `443`.
   * Port 22 is available if your network does not block it.
